    ...
    class Spawner
    {
        ...
        public Spawner(SpawnRuleDefine define, Map map)
        {
            ...
            if(DataManager.Instance.SpawnPoints.ContainsKey(this.Map.ID))
            {
                ...
                {
                    this.spawPoint = DataManager.Instance.SpawnPoints[this.Map.ID][this.Define.SpawnPoint];
                }
                else
                {
                    Log.ErrorFormat("SpawnRule[{0}] SpawnPoint[{1}] not found", this.Define.ID, this.Define.SpawnPoint);
                }
            }
        }
        ...
        public void Spawn()
        {
            ...
            this.Map.MonsterManager.Create(this.Define.SpawnMonID, this.Define.SpawnLevel, this.spawPoint.Position, this.spawPoint.Direction);
        }
    }
}
