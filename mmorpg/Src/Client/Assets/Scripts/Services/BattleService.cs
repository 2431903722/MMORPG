    ...
    class BattleService : Singleton<BattleService>, IDisposable
    {
        ...
        {           
            if(message.Result == Result.Success)
            {
                ...
                {
                    Debug.LogFormat("OnSkillCast: skillId:{0} casterId:{1} targetId:{2} position:{3} result:{4}", cast.skillId, cast.casterId, cast.targetId, cast.Position.String(), message.Result);
